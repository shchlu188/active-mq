...
public class Producer01 {
    private static final String DEFAULT_URL = "tcp://192.168.137.134:61616";
    ...
    private static void producer_01(int count) throws JMSException {
        ...
        for (int i = 0; i < count; i++) {
            ...
            textMessage.setStringProperty("stringKey","String Property");
            MapMessage mapMessage = session.createMapMessage();
            mapMessage.setStringProperty("mapStringKey","String property");
    ...
    private static void producer_jms_01(int count) throws JMSException {
        connection.start(); // 启动访问
        Session session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);
        log.info("【Sender】 send msg to queue:" + MSG);
        MessageProducer producer = session.createProducer(session.createQueue(DEFAULT_NAME));
